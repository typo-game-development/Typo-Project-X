...
namespace TC {
	...
	public class ParticleRenderer : ParticleComponent {
        ...
        public Color GetMaterialTintColor()
        {
            if(_material != null)
            {
                if(_material.HasProperty("_TintColor"))
                {
                    return _material.GetColor("_TintColor");
                }
                else
                {
                    return Color.white;
		...
		public void UpdateColourOverLifetime() {
			TextureFromGradient(ColorOverLifetime, colorOverLifetimeTexture, GetMaterialTintColor());
		}
		...
		internal void SetupDrawCall(Camera cam) {
			...
			Color tint = GetMaterialTintColor();
