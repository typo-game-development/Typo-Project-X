...
{
    [ExecuteInEditMode]
	public class BezierSplineAdvanced : MonoBehaviour
	{
		private static Material gizmoMaterial;

		private Color gizmoColor = Color.white;
		private float gizmoStep = 0.001f;
        ...
        public Vector3 GetDirection( float t)
        {
            ...
            v1 = new Vector3(lastPos.x, lastPos.y, lastPos.z);
                lastPos = GetPoint(t);
            v2 = new Vector3(lastPos.x, lastPos.y, lastPos.z);
        ...
        public Vector3 FindNearestPointToRange(Vector3 worldPos, out float normalizedT,float lowerT, float upperT, float accuracy = 100f)
        {
            Vector3 result = Vector3.zero;
            normalizedT = -1f;
            ...
            float minDistance = Mathf.Infinity;
            for (float i = lowerT; i < upperT; i += step)
            {
                Vector3 thisPoint = GetPoint(i);
                float thisDistance = (worldPos - thisPoint).sqrMagnitude;
                if (thisDistance < minDistance)
		...
		public float AccuracyToStepSize( float accuracy )
		{
			...
			return Mathf.Clamp( 1f / accuracy, 0.000001f, 0.2f );
		}
