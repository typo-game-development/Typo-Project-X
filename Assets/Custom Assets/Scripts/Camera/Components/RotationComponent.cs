...
namespace AdvancedUtilities.Cameras.Components
{
    ...
    public class RotationComponent : CameraComponent
    {
        ...
        /// </summary>
        [Tooltip("Limits the amount you can rotate in any direction based off the orientation.")]
        public IsometricRotation Isometric = new IsometricRotation();
        ...
        public bool _isoRightRotated = false;

        /// <summary>
        /// The limit that the Camera can rotate vertically below the target. Generally this should be a negative value.
        /// </summary>
        public bool _isoLeftRotated = false;

        /// <summary>
        /// Limits for horizontal rotation will be enforced.
        /// </summary>
        public bool _isoUpRotated = false;

        /// <summary>
        /// The limit that the Camera can rotate horizontally to the left of the target. Generally this should be a positive value.
        /// </summary>
        public bool _isoDownRotated = false;


        public enum eIsoRotation
        {
            UP,
            RIGHT,
            LEFT,
        ...
        /// </summary>
        public float _autoRotateTimeRemaining;
        ...
        {
            if ((UnityEngine.Cursor.lockState == CursorLockMode.None || !AutoRotation.SmartFollow.DontUpdateIfMouseLocked) &&
        ...
        public void SetIsoRotation(eIsoRotation rotation, bool flag)
        {
            if (_autoRotateTimeRemaining <= 0)
            {
                switch (rotation)
                {
                    case eIsoRotation.DOWN:
                        if (flag && !_isoDownRotated)
                        {
                            this.AutoRotateBy(0, -Isometric.DownRotation, Time.deltaTime * 18f);
                        }
                        else if (!flag && _isoDownRotated)
                        {
                            this.AutoRotateBy(0, Isometric.DownRotation, Time.deltaTime * 18f);
                    ...
                    case eIsoRotation.UP:
                        if (flag && !_isoUpRotated)
                        {
                            this.AutoRotateBy(0, Isometric.UpRotation, Time.deltaTime * 18f);
                        }
                        else if (!flag && _isoUpRotated)
                        {
                            this.AutoRotateBy(0, -Isometric.UpRotation, Time.deltaTime * 18f);
                        }
                        break;

                    case eIsoRotation.RIGHT:
                        if (flag && !_isoRightRotated)
                        {
                            this.AutoRotateBy(-Isometric.RightRotation, 0, Time.deltaTime * 18f);
                        }
                        else if (!flag && _isoRightRotated)
                        {
                            this.AutoRotateBy(Isometric.RightRotation, 0, Time.deltaTime * 18f);
                        }
                        break;

                    case eIsoRotation.LEFT:
                        if (flag && !_isoLeftRotated)
                        {
                            this.AutoRotateBy(Isometric.LeftRotation, 0, Time.deltaTime * 18f);
                        }
                        else if (!flag && _isoLeftRotated)
                        {
                            this.AutoRotateBy(-Isometric.LeftRotation, 0, Time.deltaTime * 18f);
                        }
                        break;
        ...
        public bool waitingForIsoRotation = false;

        public System.Collections.IEnumerator SetIsoRotationUpFlag(bool flagValue, float time)
        {

                waitingForIsoRotation = true;
                yield return new WaitForSeconds(time);
                if (_autoRotateTimeRemaining <= 0)
                {
                    _isoUpRotated = flagValue;
                waitingForIsoRotation = false;
            }
        }

        public System.Collections.IEnumerator SetIsoRotationDownFlag(bool flagValue, float time)
        {
                ...
                if (_autoRotateTimeRemaining <= 0)
                {
                    _isoDownRotated = flagValue;
                waitingForIsoRotation = false;
            }
        }

        public System.Collections.IEnumerator SetIsoRotationLeftFlag(bool flagValue, float time)
        {
            ...
            if (_autoRotateTimeRemaining <= 0)
            {
                _isoLeftRotated = flagValue;
                waitingForIsoRotation = false;
            }
        }

        public System.Collections.IEnumerator SetIsoRotationRightFlag(bool flagValue, float time)
        {
