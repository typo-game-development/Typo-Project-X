...
namespace FAE
{
    ...
    public class PigmentMapGenerator : MonoBehaviour
    {
        ...
        public void SetPigmentMap()
        {
            if (pigmentMap)
            {
                Shader.SetGlobalVector("_TerrainUV", new Vector4(targetSize.x, targetSize.z, Mathf.Abs(targetOriginPosition.x - 1) + targetCenterPosition.x, Mathf.Abs(targetOriginPosition.z - 1) + targetCenterPosition.z));
